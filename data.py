...
CG_ID = {
    ...
    "POL":"matic-network","POLY":"polymath-network","JST":"just",
    "SUI":"sui","INJ":"injective-protocol","TIA":"celestia",
    "WLD":"worldcoin-wld","GRT":"the-graph","OCEAN":"ocean-protocol",
    "PENDLE":"pendle","GMX":"gmx","LDO":"lido-dao","KAITO":"kaito",
    "ONDO":"ondo-finance","TON":"the-open-network","TAO":"bittensor",
    "FET":"fetch-ai","AAVE":"aave","MKR":"maker","RPL":"rocket-pool",
    "SEI":"sei-network","APT":"aptos","ARB":"arbitrum","OP":"optimism",
    "STX":"blockstack","IMX":"immutable-x","JUP":"jupiter-ag",
    "PYTH":"pyth-network","STRK":"starknet","ENA":"ethena","W":"wormhole",
}
CAP_ID = {
...
def get_crypto_panic(symbol: str) -> list:
    ...
    for p in data["results"][:5]:
        votes = p.get("votes", {})
        sentiment = "bullish" if votes.get("liked", 0) > votes.get("disliked", 0) else \
                    "bearish" if votes.get("disliked", 0) > votes.get("liked", 0) else "neutral"
        out.append({
            "title":     p.get("title", ""),
...
def _mexc_quote(symbol: str) -> dict:
    ...
    return {
        ...
        "high_24h":   float(data.get("highPrice", price) or price),
        "low_24h":    float(data.get("lowPrice", price) or price),
    }


def _gate_quote(symbol: str) -> dict:
    """Gate.io 24hr ticker — fallback for Gate-exclusive coins."""
    ...
    try:
        ...
        return {
            ...
            "high_24h":   float(t.get("high_24h", price) or price),
            "low_24h":    float(t.get("low_24h", price) or price),
        }
    except Exception as e:
        logger.debug(f"Gate quote {symbol}: {e}")
        return {}
...
# ─────────────────────────────────────────────────────────────────────────────
# 9. MACRO DATA  (FRED — official, free, requires registered API key)
# ─────────────────────────────────────────────────────────────────────────────
FRED_KEY  = os.getenv("FRED_API_KEY", "")
FRED_BASE = "https://api.stlouisfed.org/fred/series/observations"

def _fred_series(series_id: str, limit: int = 1) -> list:
    """Fetch the most recent `limit` observations for a FRED series.
    Returns [] on any failure (missing key, network error, bad series —
    FRED uses "." for not-yet-released/missing values, filtered out)."""
    if not FRED_KEY:
        return []
...
def _fred_latest(series_id: str) -> Optional[float]:
    obs = _fred_series(series_id, limit=1)
    if not obs:
        return None
    try:
...
def _macro_cached(ts: int) -> dict:
    """
    Macro context: Fed Funds Rate, US CPI (YoY), US GDP, 10Y Treasury
    yield, S&P 500, a DXY proxy, and Gold — refreshed hourly (see the
    `ts` cache-key trick in get_macro() below).

    Source: FRED (Federal Reserve Economic Data, St. Louis Fed) —
    official, genuinely free, requires a registered API key (30-second
    signup at fred.stlouisfed.org, set as FRED_API_KEY env var).

    Replaces Econdb, which added an authentication requirement at some
    point after this was first written (its own docstring said "free,
    no key for common series" — no longer true; a direct test call now
    returns {"detail":"Authentication credentials were not provided."}).
    Every fed_rate/us_cpi/us_gdp call was silently failing and returning
    null, which the frontend was masking with stale hardcoded fallback
    numbers (4.25%, 3.1%) instead of surfacing as unavailable.

    Also fixes: DXY was a literal `None` here before ("Would need forex
    API" — never implemented). Now uses FRED's DTWEXBGS (Trade Weighted
    US Dollar Index: Broad) as a free proxy — NOT numerically identical
    to the licensed ICE DXY index, but tracks the same dollar-strength
    concept; expect a different-looking number, not a bug.

    10Y yield and S&P 500 were previously pure frontend hardcoded
    constants (4.38, 5241) that never reached this function at all —
    added here now (DGS10, SP500) so they're genuinely live too.

    Returns None for any field that's genuinely unavailable rather than
    a fabricated number — the frontend shows "—" / "Unavailable" for
    null fields instead of silently substituting a stale fixed value.
    """
    macro = {
        "fed_rate": _fred_latest("FEDFUNDS"),
        "us_cpi":   _fred_yoy_pct("CPIAUCSL"),
        "us_gdp":   _fred_latest("GDP"),
        "yield10y": _fred_latest("DGS10"),
        "sp500":    _fred_latest("SP500"),
...
def get_watchlist_scores(symbols: list[str]) -> list[dict]:
    ...
    Uses Binance batch ticker endpoint (fast, no rate-limit issues).
      ...
      try:
        ...
        if r.status_code == 451:
            ...
            for sym, bnc_sym in zip(symbols, syms_usdt):
                ...
                if not t:
                    # try with global Binance if not found
                    results.append({"symbol": sym.upper(), "price": 0, "change_24h": 0, "score": 0, "signal": "WATCH"})
                    continue
                price = float(t.get("lastPrice", 0))
    ...
    try:
        ...
        logger.warning(f"get_watchlist_scores: CoinGecko fallback also failed ({e})")
...
def health_check() -> dict:
    ...
    # Twelve Data removed — indicators now computed from Binance OHLCV
    results["twelve_data"] = "removed"
