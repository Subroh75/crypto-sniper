"""
platform/registry/agent_base.py
───────────────────────────────────────────────
    ...
    bizdev   - business development (leads, outreach)
      sales    - subscriber lifecycle (TON payments, churn)
        ops      - platform health (monitoring, alerts)
          ...
          Design principles:
            Every agent has identity (department, name, version)
              Every agent declares what it reads and writes
                Every agent fails gracefully - never crashes the swarm
                  Every agent is observable - logs, metrics, errors
                    Every agent has a health check
                    """
      ...
      """
          Immutable identity declaration for every agent.
              Defined as a class attribute on every BaseAgent subclass.
                  """
      department:       str        # signal|bizdev|sales|ops|product
    name:             str        # kalman|garch|whale|lead_scanner etc
    version:          str        # semver: 1.0.0
    description:      str        # one sentence what this agent does
    reads:            list[str]  # blackboard keys it reads
    writes:           list[str]  # blackboard keys it writes
    schedule_seconds: int        # how often it runs
...
class BaseAgent(ABC):
                                                                  ...
                                                                  name="kalman",
                                                                                  version="1.0.0",
                                                                                                  description="Cleans price feed via Kalman filter",
                                                                                                                  reads=[],
                                                                                                                                  writes=["kalman_price", "kalman_velocity", "kalman_uncertainty"],
                                                                                                                                                  schedule_seconds=60,
                                                                                                                                                              )
    ...
    async def run(
              ...
              """
                      Core agent logic. Called by the scheduler.
    ...
    async def health_check(self) -> bool:
              """
                      Override to add API connectivity checks.
                              Default: always healthy.
                                      Called before each run cycle by the scheduler.
                                              """
              return True
    ...
    def _ok(
              ...
              self.logger.debug(
                  f"[{self.identity.name}] OK in {ms}ms "
    ...
    def _fail(
              ...
              """Return a failed result. Logs the error."""
              ms = int((time.monotonic() - t0) * 1000)
              self.logger.error(
                  f"[{self.identity.department}/{self.identity.name}] "
                  f"FAILED in {ms}ms: {error}"
              )
              return AgentResult(
                  success=False,
                  data={},
    ...
    def _warn(self, message: str) -> None:
              """Log a warning. Use inside run() for non-fatal issues."""
              self.logger.warning(
                  f"[{self.identity.name}] {message}"
              )
    ...
    def log_health(
              ...
              """
                      Write health status to Supabase agent_health table.
                              Called automatically by scheduler after health_check().
